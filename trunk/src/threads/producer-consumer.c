...
#define DEBUG 0 /* debug messages switch */
#define OUTPUT 0 /* output consumed character switch */
...
{
	if(DEBUG)
		printf("<state>generating %i producer + %i consumer<state>\n", num_producer, num_consumer);
...
consumer(void *aux UNUSED){
	...
	while(true)
	{
		...
		if(DEBUG) {
			printf("<state> head:%i | tail:%i | wrap:%i</state>\n", head, tail, wrap);
			printf("<consumer%i start>\n", thread_tid());
		}
