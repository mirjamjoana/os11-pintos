...
/* List of processes sleeping (officially THREAD_BLOCKED state) */
...
/*
 * Transition from a running thread T to the sleeping state.
 *
 * The Thread is waken up by the scheduler after sleeping_ticks ticks have passed.
 */
void
thread_sleep (int sleeping_ticks)
{
	...
	if (cur != idle_thread) {
		/* set sleep time */
		cur->wakeup_tick = user_ticks + kernel_ticks + idle_ticks + sleeping_ticks;
		cur->status = THREAD_READY;

		/* instead of adding thread t to ready list,
		add t to sleeping list */
		list_push_back(&sleeping_list, &cur->sleepelem);

		/* schedule new element */
		schedule ();
	}
...
/* Chooses and returns the next thread to be scheduled. Checks if
   a thread has to be waken up from sleeping mode. Should
...
next_thread_to_run (void)
{
	...
	/* check sleeping list for threads which need to be waken up */
	   ...
	   e = list_next (e))
	{
		...
		if(t->wakeup_tick <= (kernel_ticks + user_ticks + idle_ticks)) {
			...
			/* add thread to the front of the ready list, to be 
			   popped next / with higher priority. This aims at minimizing
			   the delay between the actual sleep time and the real time 
			   until returning to the thread called thread_sleep() */
